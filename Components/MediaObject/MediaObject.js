...
const MediaObject = ({ mediaTitle, mediaText, mediaImage, mediaImageAlt, navlink, anchor, link }) => {
    return (
        <>
            <Box className="media">
                <Box className={`mr-3 d-flex justify-content-center align-items-center circle p-1`} style={{height:"4rem", width:"4rem", backgroundColor:"var(--blue)", borderRadius:"50%",}}>
                ...
                <Box className='media-body'>
                    <Typography variant="h5" className='mt-0' gutterBottom>{navlink ? <Link href={link}>
                        <a className="text-decoration-none" style={{color:"var(--blue)"}}>{mediaTitle}</a>
